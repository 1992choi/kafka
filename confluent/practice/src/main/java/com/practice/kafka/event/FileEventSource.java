    ...
    public void run() {
        try {
            ...
            while (this.keepRunning) {
                ...
                if (len < this.filePointer) { // 만약 최종 filePointer가 현재 file의 length보다 작다면 file이 초기화 되었음.
                    logger.info("log file was reset as filePointer is longer than file length");
                    // 최종 filePointer를 file의 length로 설정.
                    filePointer = len;
                } else if (len > this.filePointer) { // 만약 file의 length가 최종 filePointer보다 크다면 file이 append 되었음.
                    // 최종 filePointer에서 맨 마지막 파일까지 한 라인씩 읽고 이를 Producer에서 메시지로 전송.
                    readAppendAndSend();
                } else { // 만약 file의 length와 filePointer가 같다면 file에 변화가 없음.
                    continue;
                }
            }
