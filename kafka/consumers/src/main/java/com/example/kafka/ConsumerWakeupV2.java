    ...
    public static void main(String[] args) {
        ...
        try {
            while (true) {
                ...
                try {
                    // poll() 호출 간격이 max.poll.interval.ms를 넘으면 rebalance가 수행된다.
                    // 잦은 rebalance을 피하기 위해서는 비즈니스로직의 리팩토링이 필요하고, 적절한 max.poll.interval.ms 옵션 설정이 필요하다.
                    logger.info("main thread is sleeping {} ms during while loop", loopCnt * 10_000);
                    Thread.sleep(loopCnt * 10_000);
                } catch (Exception e) {
                    e.printStackTrace();
                }
            }
